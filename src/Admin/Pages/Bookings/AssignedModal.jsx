...
const AssignedModal = ({ isOpen, setIsOpen, onSubmit }) => {
  ...
  const { data: users = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await axiosSecure(
        `/users?role=decorator&&status=available`
      );
      return data;
    },
    enabled: isOpen,
  ...
  const handleUserSelect = (decoratorId) => {
    const found = users.find((u) => u._id === decoratorId);
  ...
  const handleFinalSubmit = (data) => {
    const decoratorId = data.decorator;
    const selectedDecorator = users.find((u) => u._id === decoratorId);

    const decoratorData = {
      decoratorName: selectedDecorator.displayName,
      decoratorEmail: selectedDecorator.email,
    };

    onSubmit(decoratorData);
    reset();
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-white w-full max-w-xl rounded-2xl shadow-xl p-6 lg:p-8 relative">
        ...
        {/* Heading */}
        <h2 className="text-2xl font-semibold mb-1">Assign a Decorator</h2>
        <p className="text-base-content/60 mb-6">
          Select a decorator to assign this booking.
        </p>
        ...
        <form onSubmit={handleSubmit(handleFinalSubmit)} className="space-y-5">
          {/* SELECT DECORATOR */}
          <div>
            <label className="font-medium">Select Decorator</label>
